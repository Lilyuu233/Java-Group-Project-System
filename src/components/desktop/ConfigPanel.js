...
const ConfigPanel = forwardRef(({ setRawData, setLabels, setFile, onCompress,isCompressing, setCompressedData, setCompressedLabels, onUploadComplete  }, ref) => {
    ...
    const handleOptimize = async () => {
        ...
        const requestData = {
            data: fullLabels.map((timestamp, index) => ({
                timestamp,
                value: fullData[index]
            }))
        };

        try {
            const response = await fetch('http://localhost:5000/optimise', {
                method: 'POST',
                headers: {
            ...
            // Map Flask response to frontend parameter names
            setCompressionDeviationLimit(String(result.cf_deviation_limit));
            setCompressionDeviationType(result.cf_deviation_type);
            setExceptionFilterDeviationLimit(String(result.ef_deviation_limit));
            setExceptionFilterDeviationType(result.ef_deviation_type);
            setMinResampleLimit(String(result.min_resample_limit[0])); // Assuming single value
            setMaxResampleLimit(String(result.max_resample_limit[0])); // Assuming single value

            setMessage(`Optimisation complete${file?.name ? ` for ${file.name}` : ''}! Parameters updated.`);

            // Save optimized parameters to sessionStorage
            const optimizedParams = {
                compressionDeviationLimit: result.cf_deviation_limit,
                compressionDeviationType: result.cf_deviation_type,
                exceptionFilterDeviationLimit: result.ef_deviation_limit,
                exceptionFilterDeviationType: result.ef_deviation_type,
                minResampleLimit: result.min_resample_limit[0],
                maxResampleLimit: result.max_resample_limit[0]
            };
            sessionStorage.setItem('lastOptimizedParams', JSON.stringify(optimizedParams));
        } catch (err) {
            console.error('Error optimizing parameters:', err);
            setError(`Failed to optimize parameters: ${err.message}`);
        } finally {
    ...
    return (
        <div style={{
            ...
            {/* Data Source Section */}
            <div style={{ width: '40%', marginBottom: '10px', padding: '0 20px' }}>
                <h2 style={{
                    color: '#333',
                    fontSize: '20px',
                    fontWeight: 'bold',
                ...
                <div style={{
                    ...
                    >
                        {isOptimizing ? 'Optimizing...' : 'Optimise'}
                    </button>
                </div>
            </div>
            <div style={{
                ...
                <div style={{ width: '100%', marginBottom: '0px', padding: '0 0px' }}>
                    <div style={{ padding: '20px' }}>
                        ...
                        {/* Compression Parameters */}
                        <div style={{ ...rowStyle, gap: '20px', marginBottom: '20px' }}>
                            {/* Deviation Limit */}
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                                    ...
                                    <input
                                        ...
                                        style={{
                                            ...
                                            fontSize: '14px',
                                            marginBottom: 0,
                                            borderColor: error.includes('compressionDeviationLimit') ? 'red' : '#ccc'
                                        }}
                                    />
                                </div>
                                <span style={hintStyle}>
            Range: {VALIDATION_RANGES.compressionDeviationLimit.min}-{VALIDATION_RANGES.compressionDeviationLimit.max} · Maximum allowed deviation
        </span>
                            </div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                                    ...
                                    <input
                                        ...
                                        style={{
                                            ...
                                            fontSize: '14px',
                                            marginBottom: 0,
                                            borderColor: error.includes('exceptionFilterDeviationLimit') ? 'red' : '#ccc'
                                        }}
                                    />
                                </div>
                                <span style={hintStyle}>
            Range: {VALIDATION_RANGES.exceptionFilterDeviationLimit.min}-{VALIDATION_RANGES.exceptionFilterDeviationLimit.max} · Threshold for anomaly filtering
        </span>
                            </div>
                            ...
                            <div style={columnStyle}>
                                ...
                                <span style={hintStyle}>
            Type of deviation (default: absolute)
        </span>
                            </div>
                        </div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                                    ...
                                    <input
                                        ...
                                        style={{
                                            ...
                                            fontSize: '14px',
                                            marginBottom: 0,
                                            borderColor: error.includes('minResampleLimit') ? 'red' : '#ccc'
                                        }}
                                    />
                                </div>
                                <span style={hintStyle}>
            Range: {VALIDATION_RANGES.minResampleLimit.min}-{VALIDATION_RANGES.minResampleLimit.max} {VALIDATION_RANGES.minResampleLimit.unit} · Minimum time between samples
        </span>
                            </div>

                            {/* Max Resample Interval */}
                            <div style={columnStyle}>
                                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                                    <label
                                        style={{
                                    ...
                                    <input
                                        ...
                                        onBlur={validateMaxResampleOnBlur}
                                        ...
                                        style={{
                                            ...
                                            fontSize: '14px',
                                            marginBottom: 0,
                                            borderColor: error.includes('maxResampleLimit') ? 'red' : '#ccc'
                                        }}
                                    />
                                </div>
                                <span style={hintStyle}>
            Range: {VALIDATION_RANGES.maxResampleLimit.min}-{VALIDATION_RANGES.maxResampleLimit.max} {VALIDATION_RANGES.maxResampleLimit.unit} · Maximum time between samples
        </span>
                            </div>
                        </div>
                    </div>
