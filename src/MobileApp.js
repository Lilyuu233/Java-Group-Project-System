...
import ConfigPanel from "./components/mobile/ConfigPanel";
import MetricsTable from "./components/mobile/CompressionMatrixTable";
import CompressionSidebarPanel from "./components/mobile/CompressionSidebarPanel";
import ZoomableChart from "./components/mobile/Chart";
...
function MobileApp() {
    ...
    const handleLoadConfig = (config) => {
        ...
        setCompressedData(config.compressedData || []);
        setCompressedLabels(config.compressedLabels || []);
        if (configPanelRef.current?.loadFromCloud) {
            configPanelRef.current.loadFromCloud(config.labels || [], config.rawData || []);
        }
    ...
    const compressedDataForChart = compressedData
        .map((value, index) => {
    ...
    if (error) {
        console.warn("Failed to load configs, but continuing anyway:", error);
    }
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                <div style={{ flexGrow: 1, textAlign: 'center',marginLeft:'50px' }}>
            ...
            >
                ...
                >
                    <ConfigPanel
                        ...
                        onOptimized={(params) => {
                            setEditedConfig({
                                name: `Optimised Config - ${new Date().toLocaleTimeString()}`,
                                date: new Date().toISOString(),
                                parameters: params,
                            });
                            setModalOpen(true);
